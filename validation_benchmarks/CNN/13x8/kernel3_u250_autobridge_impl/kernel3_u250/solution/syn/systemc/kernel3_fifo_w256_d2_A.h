...
// ==============================================================
#ifndef kernel3_fifo_w256_d2_A_HH_
#define kernel3_fifo_w256_d2_A_HH_

#include <systemc>
using namespace std;
SC_MODULE(kernel3_fifo_w256_d2_A) {
    static const unsigned int DATA_WIDTH = 256;
    static const unsigned int ADDR_WIDTH = 2;
    static const unsigned int kernel3_fifo_w256_d2_A_depth = 3;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[kernel3_fifo_w256_d2_A_depth];
    ...
    SC_CTOR(kernel3_fifo_w256_d2_A) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < kernel3_fifo_w256_d2_A_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (kernel3_fifo_w256_d2_A_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < kernel3_fifo_w256_d2_A_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (kernel3_fifo_w256_d2_A_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > kernel3_fifo_w256_d2_A_depth) {
...
#endif //kernel3_fifo_w256_d2_A_HH_
