...
typedef struct {
    ...
    XKernel3_kernel3_uio_map maps[ MAX_UIO_MAPS ];
} XKernel3_kernel3_uio_info;

/***************** Variable Definitions **************************************/
static XKernel3_kernel3_uio_info uio_info;
...
static int uio_info_read_name(XKernel3_kernel3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKernel3_kernel3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKernel3_kernel3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKernel3_kernel3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKernel3_kernel3_Initialize(XKernel3_kernel3 *InstancePtr, const char* InstanceName) {
	XKernel3_kernel3_uio_info *InfoPtr = &uio_info;
