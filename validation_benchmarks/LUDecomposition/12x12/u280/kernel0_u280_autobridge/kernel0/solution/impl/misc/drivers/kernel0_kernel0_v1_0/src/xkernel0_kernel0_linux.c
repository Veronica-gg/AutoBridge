...
typedef struct {
    ...
    XKernel0_kernel0_uio_map maps[ MAX_UIO_MAPS ];
} XKernel0_kernel0_uio_info;

/***************** Variable Definitions **************************************/
static XKernel0_kernel0_uio_info uio_info;
...
static int uio_info_read_name(XKernel0_kernel0_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKernel0_kernel0_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKernel0_kernel0_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKernel0_kernel0_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKernel0_kernel0_Initialize(XKernel0_kernel0 *InstancePtr, const char* InstanceName) {
	XKernel0_kernel0_uio_info *InfoPtr = &uio_info;
