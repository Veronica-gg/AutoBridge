...
entity kernel0_kernel0_control_s_axi is
generic (
...
end entity kernel0_kernel0_control_s_axi;
...
architecture behave of kernel0_kernel0_control_s_axi is
    ...
    constant ADDR_L_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_L_DATA_1 : INTEGER := 16#20#;
    constant ADDR_L_CTRL   : INTEGER := 16#24#;
    constant ADDR_U_DATA_0 : INTEGER := 16#28#;
    constant ADDR_U_DATA_1 : INTEGER := 16#2c#;
    constant ADDR_U_CTRL   : INTEGER := 16#30#;
    ...
    signal int_U               : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_U_DATA_1 =>
                        rdata_data <= RESIZE(int_U(63 downto 32), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_U_DATA_1) then
                    int_U(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_U(63 downto 32));
                end if;
            end if;
        end if;
    end process;
